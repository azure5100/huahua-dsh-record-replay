 ...
 */
function scanZstdFrames(buffer: Buffer): { frames: ZstdFrameRange[] } {
...
function decompressFrame(frame: Buffer): Buffer {
  if (nativeSupported()) return zstdDecompressSync(frame)
 ...
 */
export function decodeZstdPrefix(input: Uint8Array, maxBytes = 128 * 1024): string {
  if (!nativeSupported()) {
    return new TextDecoder().decode(fzstdDecompress(input))
  }
  const buf = Buffer.isBuffer(input) ? input : Buffer.from(input)
