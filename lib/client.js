	...
	factory: (require) => {
		...
		var ReplayApi = class {
			async createRecording(title) {
				return (await readJson(await fetch("/api/dsh-record-replay/recordings", {
					method: "POST",
			...
			async uploadRecordingVideo(id, blob) {
				await readJson(await fetch("/api/dsh-record-replay/recording" + query({
					id,
					part: "video"
				}), {
					method: "POST",
			...
			async uploadRecordingFrame(id, name, blob) {
				return (await readJson(await fetch("/api/dsh-record-replay/recording" + query({
					id,
					name
				}), {
					method: "POST",
			...
			async listRecordings() {
				return (await readJson(await fetch("/api/dsh-record-replay/recordings"))).recordings;
			}
			async getRecording(id) {
				return (await readJson(await fetch("/api/dsh-record-replay/recording" + query({ id })))).recording;
			}
			async deleteRecording(id) {
				await readJson(await fetch("/api/dsh-record-replay/recording" + query({ id }), { method: "DELETE" }));
			}
			/** URL of a recording's video (Range-enabled, for <video> playback). */
			videoUrl(id) {
				return "/api/dsh-record-replay/video" + query({ id });
			}
			/** URL of one sampled frame (usable by the describe_image tool). */
			frameUrl(id, name) {
				return window.location.origin + "/api/dsh-record-replay/frame" + query({
					id,
					name
				});
			}
			async installSkill(content) {
				return (await readJson(await fetch("/api/dsh-record-replay/skills", {
					method: "POST",
					headers: { "content-type": "application/json" },
			...
			async listSkills() {
				return (await readJson(await fetch("/api/dsh-record-replay/skills"))).skills;
			}
			async listSessions() {
				return (await readJson(await fetch("/api/dsh-record-replay/sessions"))).sessions;
			}
			async getSession(id) {
				return await readJson(await fetch("/api/dsh-record-replay/session" + query({ id })));
			}
			/** Trigger a replay-pack download for one session (browser download). */
			exportPack(sessionId, notes) {
				const url = "/api/dsh-record-replay/export" + query({
					sessionId,
					notes
				});
			...
			async listPacks() {
				return (await readJson(await fetch("/api/dsh-record-replay/packs"))).packs;
			}
			async importPack(pack) {
				return (await readJson(await fetch("/api/dsh-record-replay/packs", {
					method: "POST",
					headers: { "content-type": "application/json" },
			...
			async getPack(id) {
				return (await readJson(await fetch("/api/dsh-record-replay/pack" + query({ id })))).pack;
			}
			async deletePack(id) {
				await readJson(await fetch("/api/dsh-record-replay/pack" + query({ id }), { method: "DELETE" }));
			}
		};
		//#endregion
		...
		*/
		function placeEntry(root, entry) {
			...
			if (entry.parentElement !== root) {
				...
				const family = Array.from(root.children).filter((el) => el instanceof HTMLElement && el.matches("[data-dsh-taskboard-entry], [data-dsh-ssh-entry], [data-dsh-record-replay-entry]"));
