/**
 * The /api/huahua-dsh-record-replay route family: session library, timeline reads,
...
import { parseTimeline, extractUserMessages } from "./timeline.js";
import { buildReplayPack, parseReplayPack, packFileName, serializeReplayPack } from "./replay-pack.js";
/** API base path (the browser half fetches these same-origin). */
export const API_BASE = '/api/huahua-dsh-record-replay';
...
function serveVideo(req, res, file, size) {
    ...
    return [
        ...
        // ------------------------------------------------------- export
        {
            ...
            handler: async (req, res) => {
                ...
                if (id === undefined) {
                    writeJson(res, 400, { error: 'sessionId query parameter is required' });
                    return;
                }
                try {
                    const log = await sessions.read(id);
        ...
        // ------------------------------------------------ recordings
        {
            ...
            handler: async (req, res) => {
                ...
                if (method === 'POST') {
                    const body = await readJsonBody(req);
                    const title = typeof body === 'object' && body !== null && typeof body.title === 'string'
                        ? body.title
                        : 'untitled recording';
                    try {
                ...
                try {
                    if (method === 'POST') {
                        ...
                        if (part === 'video') {
                            ...
                            if (body === undefined) {
                                writeJson(res, 413, { error: 'video body too large' });
                                return;
                            }
                        ...
                        if (body === undefined) {
                            writeJson(res, 413, { error: 'frame body too large' });
                            return;
                        }
                    ...
                    if (method === 'GET') {
                        ...
                        if (meta === undefined) {
                            writeJson(res, 404, { error: 'recording not found' });
                            return;
                        }
        ...
        },
        {
            ...
            handler: async (req, res) => {
                ...
                if (size <= 0) {
                    writeJson(res, 404, { error: 'video not found' });
                    return;
                }
        ...
        },
        {
            ...
            handler: async (req, res) => {
                ...
                if (id === undefined || name === undefined) {
                    writeJson(res, 400, { error: 'id and name query parameters are required' });
                    return;
                }
                const file = recordings.framePath(id, name);
                if (file === undefined) {
                    writeJson(res, 400, { error: 'invalid frame name' });
                    return;
                }
                try {
        ...
        // ------------------------------------------------------ skills
        {
            ...
            handler: async (req, res) => {
                ...
                if (method === 'POST') {
                    const body = await readJsonBody(req);
                    const content = typeof body === 'object' && body !== null && typeof body.content === 'string'
                        ? body.content
                        : '';
                    if (content === '') {
